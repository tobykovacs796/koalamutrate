...
def nb_dn(direct, direct_denovo, direct_handling, name, GQ_lim, DP_min, DP_max, AB_max, AB_min):
	...
	cat_cmd = "cat {}rscript_nb_denovo.R >>{}{}_nb_denovo.r".format(direct_denovo, direct_denovo, name)
	"""Run r script"""
	r_cmd = "Rscript {}{}_nb_denovo.r \n".format(direct_denovo, name)
	"""Create a .pbs files with the filter functions."""
	file = open('{}{}_nb_denovo.pbs'.format(direct_denovo, name),'w')
	file.write('#!/bin/bash \n')
	file.write('#PBS -P RDS-FSC-Nocticola-RW \n')
	file.write('#PBS -l select=1:ncpus=10:mem=25GB \n')
	...
	sub_cmd = "qsub -o {}{}_nb_denovo.out {}{}_nb_denovo.pbs".format(direct_denovo, name, direct_denovo, name)
