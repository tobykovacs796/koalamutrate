...
def call(direct_denovo, direct_handling, name, GQ_lim, DP_min, DP_max):
	...
	"""Create a .pbs files with the BCF functions and grep the number of sites."""
	file = open('{}callability_{}.pbs'.format(direct_denovo, name),'w')
	file.write('#!/bin/bash \n')
	file.write('#PBS -P RDS-FSC-Nocticola-RW \n')
	file.write('#PBS -l select=1:ncpus=10:mem=64GB \n')
	...
	sub_cmd = "qsub {}callability_{}.pbs".format(direct_denovo, name, direct_denovo, name)
