...
def back_handling(direct, name):
	...
	"""Create a .pbs files with all the functions."""
	file = open('{}back_handling_{}.pbs'.format(direct, name),'w')
	file.write('#!/bin/bash \n')
	file.write('#PBS -P RDS-FSC-BacCock-RW \n')
	file.write('#PBS -l select=1:ncpus=1:mem=250GB \n')
...
if all(list_exist):
    ...
    mv_log= "mv -t {}log_file/ {}back*.out {}back*.pbs".format(direct, direct, direct)
