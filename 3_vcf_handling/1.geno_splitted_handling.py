...
def geno_handling(ref, geno_file, direct, output_snp, output_filt, pedigree, output_mv, output_table, name):
	...
	sfilt_cmd = "gatk VariantFiltration "
	...
	file = open('{}geno_handling_{}.pbs'.format(direct, name),'w')
	file.write('#!/bin/bash \n')
	file.write('#PBS -P RDS-FSC-BacCock-RW \n')
	file.write('#PBS -l select=1:ncpus=1:mem=100GB \n')
...
if all(list_exist):
    ...
    for name in trio_dir:
      ## move the log files:
        mv_log= "mv -t {}log_file/ {}genotype_genomicDBI_{}.out {}genotype_genomicDBI_{}.pbs".format(direct, direct, name, direct, name)
