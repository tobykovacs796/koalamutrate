...
for name in bamfile_dir:
    print(name)
    if os.path.exists("{}{}.uniq.rmdup.bam".format(bam_dir, name)):
        print("\t Rmdup and uniq exists for {} ".format(name))
        if os.path.exists("{}{}.uniq.rmdup.bam.bai".format(bam_dir, name)):
            ...
            mv_bam1 = "mv {}{}.bam {}inter_bam/".format(bam_dir, name, bam_dir)
            subprocess.call(mv_bam1, shell=True)
            mv_bam2 = "mv {}{}.uniq.bam {}inter_bam/".format(bam_dir, name, bam_dir)
    ...
    if os.path.exists("{}{}.uniq.rmdup.recal.bam".format(bam_dir, name)):
        print("\t Recalibrated bam exists for {} ".format(name))
        if os.path.exists("{}{}.uniq.rmdup.recal.bai".format(bam_dir, name)):
            print("\t and has been indexed ".format(name))
    ...
    if os.path.exists("{}{}_coverage.txt".format(cov_dir, name)) and os.path.exists("{}{}_coverage.pbs".format(cov_dir, name)):
