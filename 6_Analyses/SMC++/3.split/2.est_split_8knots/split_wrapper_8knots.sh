...
for ((i=0; i<${#populations[@]}; i++)); do
    for ((j=i+1; j<${#populations[@]}; j++)); do
        pop1=${populations[i]}
        pop2=${populations[j]}
        log_name="log2.est_splt_${pop1}_${pop2}_8knots"
        job_name="2.est_split_${pop1}_${pop2}_8knots"
        output_dir="${pop1}_${pop2}_8knots"
...
smc++ split --timepoints 1e3 1e6 -o $output_dir/ \\
    ../estimate/$pop1/tpe3e6_8knots/model.final.json \\
    ../estimate/$pop2/tpe3e6_8knots/model.final.json \\
...
smc++ plot -c split_${pop1}_${pop2}_8knots.pdf \\
